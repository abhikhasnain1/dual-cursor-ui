class_name DualCursorVerticalSliderAdapter
extends VSlider

@export var owner_player_id: int = -1
@export var interaction_enabled: bool = true
@export var hit_priority: int = 0
@export var activation_step_multiplier: float = 1.0
...
func dual_cursor_navigate(player_id: int, direction: Vector2, cursor: Node) -> bool:
	if abs(direction.y) <= abs(direction.x):
		return false
	adjust_by_player(player_id, -direction.y, cursor)
	return true
...
func adjust_by_player(player_id: int, direction: float, cursor: Node) -> void:
	...
	if not reason.is_empty():
		return
	var delta: float = _step_size() * activation_step_multiplier * sign(direction)
...
func _step_size() -> float:
	if step > 0.0:
		return step
	return max(1.0, (max_value - min_value) * 0.05)
